Feature: SMTP with IMAP access
  ...
  Scenario: Send e-mail directly to unauthenticated SMTP and check it over TLS authenticated IMAP
    ...
    Then I should find it in test.solsoft.pl IMAP inbox authenticated with LOGIN kazuya@test.solsoft.pl:test21ds34 over TLS connection within 5 seconds
  ...
  @send
  Scenario: Send e-mail to TLS authenticated SMTP and check it has arrived in external TLS authenticated IMAP
    ...
    Then I should find it in mail.sigquit.net IMAP inbox authenticated with LOGIN mailtest:test21ds35 over TLS connection within 5 seconds
